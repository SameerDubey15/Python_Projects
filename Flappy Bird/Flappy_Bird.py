...
def isCollide(playerx, playery, upperPipes, lowerPipes):
    ...
    for pipe in upperPipes:
        ...
        if(playery < pipeHeight + pipe['y'] and abs(playerx - pipe['x']) < GAME_SPRITES['pipe'][0].get_width()*0.8):
            GAME_SOUNDS['hit'].play()
            return True

    for pipe in lowerPipes:
        if (playery + GAME_SPRITES['player'].get_height() > pipe['y']) and abs(playerx - pipe['x']) < GAME_SPRITES['pipe'][0].get_width()*0.8:
            GAME_SOUNDS['hit'].play()
            return True
